...
async function replaceInFiles() {
    ...
    const codeFiles = glob.sync("**/*.{js,jsx,ts,tsx,css,scss,html}", { ignore: ["node_modules/**", "build/**", "dist/**", "webpify.js", "pre-render.js", "webpify-node.js", "webpify-bun.js"] });
            ...
            for (const conversion of convertedFiles) {
                ...
                while ((match = regex.exec(content)) !== null) {
                    ...
                    if (absoluteMatchedPath === absoluteOriginalPath) {
                        ...
                        const relativeWebPPath = path.relative(path.dirname(filePath), webp).split(path.sep).join(path.posix.sep); // Ensure forward slashes
...
async function convertImage(imagePath, outputPath) {
    try {
        await sharp(imagePath).webp({ quality: 80 }).toFile(outputPath);

        return { original: imagePath, webp: outputPath, status: "converted" };
    } catch (error) {
        console.error(`Error converting ${imagePath}:`, error);
        return { original: imagePath, webp: outputPath, status: "error", error: error.message };
    }
}
...
async function convertToWebP() {
    try {
        ...
        const images = glob.sync("**/*.{jpg,jpeg,png}", { ignore: ["node_modules/**", "build/**", "dist/**", "public/favicon.png", "public/assets/banner.png", "public/assets/bannerDEJTENÁMVAŠEVŠECHNYPRACHY.png", "public/assets/bannerNECHCETEMIPODEPSATPETICI.png", "public/assets/bannerSRAZ2025.png"] }).map(normalizePath);
        ...
        for (const imagePath of images) {
            ...
            try {
                ...
                convertedFiles.push({ original: imagePath, webp: outputPath, status: "skipped" });
            } catch {
        ...
        // Process images in parallel with Bun's improved concurrency
        if (imagesToProcess.length > 0) {
            let completedCount = 0;
            const totalToProcess = imagesToProcess.length;

            // Process in batches according to the number of threads
            for (let i = 0; i < imagesToProcess.length; i += numThreads) {
                const batch = imagesToProcess.slice(i, i + numThreads);
                const results = await Promise.all(
                    batch.map(async ({ imagePath, outputPath }) => {
                        const result = await convertImage(imagePath, outputPath);
                        completedCount++;
                        if (result.status === "converted") {
                            ...
                            console.error(`Error [${String(completedCount).padStart(totalToProcess.toString().length, "0")}/${totalToProcess}]: Failed to convert ${result.original}`);
                        }
                        return result;
                    })
                );
        ...
        await fs.writeFile("webp-conversion-results.json", JSON.stringify({ originalFiles, conversions: convertedFiles }, null, 2));
