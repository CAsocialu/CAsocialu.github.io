...
if (!isMainThread) {
    ...
    async function convertToWebP() {
        try {
            ...
            const images = glob.sync("**/*.{jpg,jpeg,png}", { ignore: ["node_modules/**", "build/**", "dist/**", "public/favicon.png", "public/assets/banner.png", "public/assets/bannerDEJTENÁMVAŠEVŠECHNYPRACHY.png"] }).map(normalizePath);
                ...
                for (let i = 0; i < imagesToProcess.length; i += numThreads) {
                    ...
                    const workerPromises = chunk.map(({ imagePath, outputPath }) => {
                        return new Promise((resolve) => {
                            const worker = new Worker(__filename, { workerData: { imagePath, outputPath, quality: 80 } });
                            ...
                            worker.on("error", (error) => {
                                ...
                                convertedFiles.push({ original: imagePath, webp: outputPath, status: "error", error: error.message });
                                resolve();
                            });
