...
function rawr(event) {
    ...
    if (stringcolonthree.includes(":3")) {
        ...
        rawrx3.innerHTML = `<img id="whOwOps" src="/assets/whOwOps/whOwOps0.png" style="aspect-ratio: 16/9; max-height: 100%; max-width: 100%;" draggable="false">`;
        ...
        setTimeout(
            () => {
                document.querySelector("img#whOwOps").setAttribute(
                    "src",
                    document
                        .querySelector("img#whOwOps")
                        .getAttribute("src")
                        .replace(/(\D+)(\d+)(\.png$)/, (match, prefix, num, suffix) => `${prefix}${parseInt(num, 10) + 1}${suffix}`)
                );
                setTimeout(
                    () => {
                        document.querySelector("img#whOwOps").setAttribute(
